...
var LysisUtils = function LysisUtils() {
  ...
  * @example lysisUtils.toCamelCase('api/good-morning_world') returns 'apiGoodMorning_world'
  * @example lysisUtils.toCamelCase('HeLlO-WorLd') returns 'helloWorld'
  ...
  */
  this.toCamelCase = function(text, whichCase) {
    ...
    result = text.split(sepRe).map(function (word) {
  ...
  * Copy a file
  *
  * When config.dryRun is true, the file copy is only mentioned (i.e.
  * not written)
  ...
  */
  this.copyFile = function(src, dest) {
    if (!fs.existsSync(src)) {
      this.notifier.emit('error', `The file to copy "${src}" does not exist.`);
    }
