...
var parseHydraDocumentation = require('@api-platform/api-doc-parser/lib/hydra/parseHydraDocumentation').default;
...
var LysisCore = function LysisCore() {
  ...
  var startGenerators = function(apiConfig, api) {
    ...
    for (var i = 0 ; i < generators.length ; i++) {
      ...
      notifier.emit('generator-start', { generator: generators[i] });
      try {
        require(generators[i])(parameters);
      } catch(e) {
        notifier.emit('generator-error', {generator: generators[i], error: e});
      }
      lysisUtils.resetTemplates();
      notifier.emit('generator-stop', { generator: generators[i] });
    }
    notifier.emit('api-stop', api.entrypoint);
  };
